...
const { confirmPasswordError_text } = require('./settings.page.js');
...
module.exports = {
	...
	sectiongroup: selectorFile.css.ComproEngage.indexMenu.sectiongroup,
	resoursebtn: selectorFile.css.ComproEngage.indexMenu.resoursebtn,
	materialbtn: selectorFile.css.ComproEngage.indexMenu.materialbtn,
	indexproductbtn: selectorFile.css.ComproEngage.indexMenu.indexproductbtn,
	currentlbl: selectorFile.css.ComproEngage.indexMenu.currentlbl,
	infoBtn: selectorFile.css.ComproEngage.tocMenu.infoBtn,
	infoTocHeading: selectorFile.css.ComproEngage.tocMenu.infoTocHeading,
	closeInfoBtn: selectorFile.css.ComproEngage.tocMenu.closeInfoBtn,
	chapterTitle: selectorFile.css.ComproEngage.tocMenu.chapterTitle,
	productbtn: selectorFile.css.ComproEngage.tocMenu.productbtn,
		...
		if (productbtnName) {
			for (var i = 0; i < list.length; i++) {
				if ((await action.getText(this.indexproductbtn + i)) == productbtnName) {
					obj[0] = {
						productbtn: ((await action.getElementCount(this.indexproductbtn + i + "] p")) > 0) ? await action.getText(this.indexproductbtn + i + "] p") : null,
	...
	click_productbtn: async function (productbtnName) {
		await logger.logInto(await stackTrace.get());
		var i, list, res;
		list = await action.findElements(this.productbtn);
		console.log(list.length)
		for (i = 0; i < list.length; i++) {
			console.log(action.getText(this.productbtn + i + "]"))
			if (((await action.getText(this.productbtn + i + "]"))) == productbtnName) {
				res = await action.click(list[i]);
				break;
			}
		}
		if (res == true) {
			await logger.logInto(await stackTrace.get(), " --productbtn clicked");
		}
		else
			await logger.logInto(await stackTrace.get(), " --productbtn NOT clicked", "error")
		return res;
	},
