...
module.exports = {
    ...
    click_yesRadio_btn: function () {
        logger.logInto(stackTrace.get());
        var res;
        res = action.click(this.requestReattempted_yes_btn);
        if (true == res) {
            logger.logInto(stackTrace.get(), " requestReattempted_yes_btn is clicked");
        }
        else {
            logger.logInto(stackTrace.get(), res +"submitDialog_yes_btn is NOT clicked", 'error');
        }
        return res;
    },

    //Function to click on 'No' button
    click_noRadio_btn: function () {
        logger.logInto(stackTrace.get());
        var res;
        res = action.click(this.requestReattempted_no_btn);
        if (true == res) {
            logger.logInto(stackTrace.get(), " requestReattempted_no_btn is clicked");
        }
        else {
            logger.logInto(stackTrace.get(), res +"requestReattempted_no_btn is NOT clicked", 'error');
        }
        return res;
    },
