...
module.exports = {
    ...
    totalMaterialCount: selectorFile.css.ComproEngage.myMaterials.totalMaterialCount,
    materialCountSubtitle: selectorFile.css.ComproEngage.myMaterials.materialCountSubtitle,
    duplicateBtn: selectorFile.css.ComproEngage.myMaterials.duplicateBtn,
    editBtn: selectorFile.css.ComproEngage.myMaterials.editBtn,
    clearSearch: selectorFile.css.ComproEngage.myMaterials.clearSearch,
    ...
    getData_myMaterialsPage: async function () {
        ...
        obj = {
            ...
            totalMaterialCount: ((await action.getElementCount(this.totalMaterialCount)) > 0) ? await action.getText(this.totalMaterialCount) : null,
            materialCountSubtitle: ((await action.getElementCount(this.materialCountSubtitle)) > 0) ? await action.getText(this.materialCountSubtitle) : null,
    ...
    getData_quizRow: async function (quizRowName) {
        await logger.logInto(await stackTrace.get());
        var obj = [];
        await action.waitForDisplayed(this.materialsQuizTitleTxt);
        var list = await action.findElements(this.materialsQuizTitleTxt);
        if (quizRowName) {
            for (var i = 0; i < list.length; i++) {
                if ((await action.getText(this.materialsQuizTitleTxt + i + "] p")) == quizRowName) {
                    obj[0] = {
                        materialsQuizTitleTxt: ((await action.getElementCount(this.materialsQuizTitleTxt + i + "]")) > 0) ? await action.getText(this.materialsQuizTitleTxt + i + "] p") : null,
                        ...
                        materialsContextMenuBtn: ((await action.getElementCount(this.materialsContextMenuBtn + i + "]")) > 0) ? await action.waitForExist(this.materialsContextMenuBtn + i + "]") : null,
                    }
                    break;
                }
            }
    ...
    click_quizRow: async function (quizRowName) {
        await logger.logInto(await stackTrace.get());
        var i, list, res;
        list = await action.findElements(this.materialsQuizTitleTxt);
    ...
    close_preview: async function () {
        await logger.logInto(await stackTrace.get());
        var res;
        res = await action.click("[data-tid=button-closestudentview]");
        if (true == res) {
            await logger.logInto(await stackTrace.get(), " previewCloseBtn is clicked");
        }
        else {
            await logger.logInto(await stackTrace.get(), res + " previewCloseBtn is NOT clicked", 'error');
        }
        return res;
    }
}
