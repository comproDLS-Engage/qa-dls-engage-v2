...
module.exports = {
	...
	isInitialized: async function (mcmrQuesData) {
		...
		if (res.value == 'solid') {
			await logger.logInto(await stackTrace.get(), " -- Choice " + mcmrQuesData[2][0] + " is of MCMR type");
			let choiceSvgSelector = choiceSelector + " svg"
			res = await action.getAttribute(choiceSvgSelector, 'data-icon');
			if (res == 'square') {
				await logger.logInto(await stackTrace.get(), " -- choice " + mcmrQuesData[2][0] + " is having square icon");
